...
const STEPS = [
    ...
    },
    {
        ...
        title: "Le don",
        description:
            "Installé·e confortablement, le prélèvement à proprement parler est l'étape la plus rapide du parcours.",
    },
    {
        icon: Utensils,
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="max-w-2xl">
                    ...
                    <p className="mt-4 text-body-lg text-tertiary">
                        De l&apos;accueil à la collation, voici à quoi vous attendre — et comment vous y
                        préparer au mieux.
                    </p>

                    <div className="mt-6 inline-flex items-start gap-2 rounded-full bg-primary-50 px-4 py-2 text-small font-medium text-primary-600">
                        <Clock className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" />
                        Le temps sur place varie selon l&apos;affluence du centre — le prélèvement
                        lui-même reste la partie la plus courte du parcours.
                    </div>
                </div>
                    ...
                    {STEPS.map((step, i) => (
                        <li key={step.title} className="flex gap-4 sm:gap-6">
                            <div className="flex flex-col items-center">
                                ...
                                {i < STEPS.length - 1 && (
                                    <span className="mt-2 w-px flex-1 bg-neutral-200" aria-hidden="true" />
                                )}
                            </div>
                            <div className="pb-2">
                                <h3 className="text-h4 text-secondary">{step.title}</h3>
                                <p className="mt-1.5 max-w-xl text-body text-tertiary">{step.description}</p>
                            </div>
                        </li>
                    ))}
                </ol>
                                ...
                                <ul className="mt-3 space-y-2">
                                    {items.map((item) => (
                                        <li key={item} className="flex gap-2 text-body text-tertiary">
                                            <span className="mt-2.5 h-1 w-1 shrink-0 rounded-full bg-primary-400" aria-hidden="true" />
                                            {item}
                                        </li>
                                    ))}
                                </ul>
